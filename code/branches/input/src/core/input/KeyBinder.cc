...
namespace orxonox
{
    ...
    */
        ...
        : deriveTime_(0.0f)
    {
        ...
        {
            joyStickButtons_[i + 0].name_ = "JoyPOV" + convertToString((i - 32)/4 + 1) + "North";
            joyStickButtons_[i + 1].name_ = "JoyPOV" + convertToString((i - 32)/4 + 1) + "South";
            joyStickButtons_[i + 2].name_ = "JoyPOV" + convertToString((i - 32)/4 + 1) + "East";
            joyStickButtons_[i + 3].name_ = "JoyPOV" + convertToString((i - 32)/4 + 1) + "West";
        }
        ...
        for (unsigned int i = 4; i < nHalfAxes_s/2; i++)
            rawNames[i] = "JoyAxis" + convertToString(i - 3);
    ...
    */
    ...
    {
        SetConfigValueGeneric(KeyBinder, analogThreshold_, 0.05f)
            .description("Threshold for analog axes until which the state is 0.");
        SetConfigValueGeneric(KeyBinder, mouseSensitivity_, 1.0f)
            .description("Mouse sensitivity.");
        SetConfigValueGeneric(KeyBinder, bDeriveMouseInput_, false)
            .description("Whether or not to derive moues movement for the absolute value.");
        SetConfigValueGeneric(KeyBinder, derivePeriod_, 0.05f)
            .description("Accuracy of the mouse input deriver. The higher the more precise, but laggier.");
        SetConfigValueGeneric(KeyBinder, mouseSensitivityDerived_, 1.0f)
            .description("Mouse sensitivity if mouse input is derived.");
        SetConfigValueGeneric(KeyBinder, bClipMouse_, true)
            .description("Whether or not to clip absolute value of mouse in non derive mode.");

        float oldThresh = buttonThreshold_;
        SetConfigValueGeneric(KeyBinder, buttonThreshold_, 0.80f)
    ...
    void KeyBinder::readTrigger(Button& button)
    {
        ...
        ConfigValueContainer* cont
            = ClassIdentifier<KeyBinder>::getIdentifier()->getConfigValueContainer(button.name_);
        if (!cont)
        {
            cont = new ConfigValueContainer
                (CFT_Keybindings, ClassIdentifier<KeyBinder>::getIdentifier(), button.name_, "");
