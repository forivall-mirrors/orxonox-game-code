...
MACRO(SET_CACHE _varname _type _docstring)
  ...
  ENDIF(NOT "${_type}" MATCHES "^(STRING|BOOL|PATH|FILEPATH)$")
  ...
  ENDIF(NOT DEFINED _INTERNAL_${_varname} OR "${_INTERNAL_${_varname}}" STREQUAL "${${_varname}}")
...
MACRO(_INTERNAL_PARSE_FLAGS _mode _flags _varname _write_to_cache _force)
  ...
  IF(_write_to_cache)
    ...
    ENDIF(_force)
  ELSE(_write_to_cache)
    SET(${_varname} "${_flag_list}")
  ENDIF(_write_to_cache)
...
MACRO(_INTERNAL_PARSE_FLAGS_ARGS _mode _keys _key_postfix _flags)
  ...
  FOREACH(_arg ${ARGN})
	...
	ELSE(${_arg})
          SET(_arg_cond TRUE)
	ENDIF(${_arg})
      ELSE(_invert_condition)
        SET(_arg_cond ${${_arg}})
      ENDIF(_invert_condition)
      IF(_cond AND _arg_cond)
        SET(_cond TRUE)
      ELSE(_cond AND _arg_cond)
        SET(_cond FALSE)
      ENDIF(_cond AND _arg_cond)
    ENDIF(_arg MATCHES "${_key_regex}")
  ENDFOREACH(_arg)

  # No language specified, use all: C and CXX or EXE, SHARED and MODULE
  IF(NOT DEFINED _langs)
    SET(_langs ${_keys})
  ENDIF(NOT DEFINED _langs)
