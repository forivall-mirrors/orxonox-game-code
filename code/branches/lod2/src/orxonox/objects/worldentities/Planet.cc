...
namespace orxonox
{
     ...
     * @brief Constructor
     */
    Planet::Planet(BaseObject* creator): PositionableEntity(creator)
    {
    ...
    void Planet::tick(float dt)
    {
        ...
        /*
        if(ticker >= 500){
            COUT(2) << "x: " << test->getWorldPosition().x << ", y: " << test->getWorldPosition().y << ", z: " << test->getWorldPosition().z << std::endl;
            COUT(2) << newScale << " " << distance << " " << tempTest << std::endl;
            ticker = 0;
        }
    ...
    void Planet::init(){
        ...
        billboard_.setBillboardSet(this->getScene()->getSceneManager(), "planet/Atmosphere", Vector3(0,0,0));
